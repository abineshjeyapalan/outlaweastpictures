...
2. **Connect a KV store**: Project → Storage → Create Database → **KV**.
   Attach it to this project. Vercel injects `KV_REST_API_URL` /
...
Once that's done, edits made in `/admin.html` are saved to KV and visible to
every visitor, not just the browser that made them.

## Without a backend

The site works fine without any of the above — if `api/content.js` isn't
reachable or the KV store / `ADMIN_SECRET` aren't configured, everything
...
between browsers or into the KV store's history.
